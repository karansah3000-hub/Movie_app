...
if opt=="Home":
   ...
   #  st.caption("🎥 From ratings and genres to streaming trends and hidden gems — discover cinema through the power of visualization.")
    ...
    with viz4:
        ...
        fig = px.line_polar(comparison_long,r="Score",theta="Platform",color="Metric",line_close=True)
        st.plotly_chart(fig)

        fig = px.imshow(platform_compare.set_index("Platform"),text_auto=".1f",aspect="auto")
        st.plotly_chart(fig)
    ...
    with viz5:
        ...
        fig = px.line(movies_year,x="Year",y="Movies",markers=True)
        st.plotly_chart(fig,use_container_width=True)
        ...
        fig = px.histogram(df,x="Decade",color="Decade")
        st.plotly_chart(fig)

        trend = df.groupby("Year")["IMDb_100"].mean().reset_index()
        fig = px.line(trend,x="Year",y="IMDb_100",markers=True)
        st.plotly_chart(fig)
        ...
        fig = px.pie(lang,names="Language",values="count",hole=0.45)
        st.plotly_chart(fig)
